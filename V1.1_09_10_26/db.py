...
def init_db():
    """Create every table above if it doesn't already exist. Safe to
    call every time the app starts (that's why app.py just calls this
    unconditionally on startup) — CREATE TABLE IF NOT EXISTS does
    nothing at all if the table's already there."""
    conn = get_db()
    conn.executescript(SCHEMA)  # executescript runs a string containing MULTIPLE SQL statements at once
